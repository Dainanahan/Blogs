...
## Objective
*DrugBank* is a well-known repository containing high-quality information on 
drugs and their known targets. The latest release of the *DrugBank* database is
available for download in XML format 
[here](https://www.drugbank.ca/releases/latest). 

In this post, we shall take a journey together to explore the **structure** of 
the *DrugBank* database. We will observe how the drugs information is 
...
containing XML file. Let's begin!

-----

## What is *DrugBank*?

<img style="float:right" src="DrugBank_Logo.png" width="50%"/>
...
[XML file](drugbank_record.xml). It is a dummy XML file that contains only a 
single drug record (i.e. a single `<drug>` node).

<p style="color:red;">
Note that the file, `drugbank_record.xml` is intended to be downloadable by the 
user so that they may follow along with this tutorial.
</p>
...
beside its associated attributes -- many *children* nodes with varying names, 
properties and, more importantly to us, *structures*. 
...
```
<p style="color:red;">
Figure shows wierd tooltips when the mouse cursor hovers above it. Why?
</p>
<p style="color:red;">
Figure does not adjust its size to width of screen. Why?
</p>
...
value). To that end, we will use the following functions of the `purrr` and 
`XML` packages: 
    
* `xmlValue()`: gets the *content* of a given node
* `xmlGetAttr()`: gets a specified *attribute* of a given node
...
## <drugbank> parent node
drugbank <- ???('drugbank_database.xml')
```

<p style="color:red;">
Suggestion: consider adding this line of code: `all_drugs <- drugbank$children` 
then use `all_drugs` afterwards
</p>

Now that XML file has been loaded, let's extract the drugs' data. The 
`map_df()` function takes two parameters:

* `drugbank$children`: As already mentioned, there is a single `<drugbank>` 
  node in the XML file, and its children are the `<drug>` nodes that contain 
  all the drugs data that we are interested in.
* `drug_df`: the method that we created above for the extraction of the drugs' 
  data from their corresponding `<drug>` nodes

```{r extract_all_drug_info, eval=FALSE}
## '~' is telling map_df that we are sending the function a dynamic parameter 
## that is based on its first parameter
drugs <- map_df(drugbank$children, ~drug_df(.x))
```

<p style="color:red;">
`drugs <- map_df(drugbank$children, drug_df)`
</p>
<p style="color:red;">
Wouldn't this line work? If yes, use instead of the one above?
</p>
...
drug_groups <- map_df(drugbank$children, ~drug_sub_df(.x, "groups"))
drug_products <- map_df(drugbank$children, ~drug_sub_df(.x, "products"))
```
...
In the above example, the nodes that we parsed consist of a groups of similar 
nodes; that is:

* a `<groups>` node consists of multiple `<group>` nodes
* a `<products>` node consists of multiple `<product>` nodes
...
### Optional parameters
We have two optional parameters which are discussed below.

#### sub_child_node
We use this parameter if we are going to address the child by two names like:

```{r sec_param, eval=FALSE, highlight=TRUE}
# Here we need the drug articles only inside the drug references
articles <- drug_sub_df(.x, "general-references", sub_child_node = "articles")
```
...
```{r sec_param_xml, eval=FALSE, highlight=TRUE}
<general-references>
    <articles>
        <article>
...
#### id
As already implied earlier, this field is used to determine the `parent_key` 
field (by default, `drugbank-id`) to attach to the extracted data. As an 
example, we parse ???.

<p style="color:red;">
Add an example here, and actually run it (i.e. `eval=TRUE`) on the XML file 
provided by us earlier (`drugbank_record.xml`), and then display the first few 
records of the result (using the `head()` function). It is okay if you feel 
like using two examples, not just one.
</p>

```{r third_param, eval=FALSE, highlight=TRUE}
# # get actions inside carriers nodes,
# # the id here is carrier node
# carriers_actions <- drug_sub_df(.x, "actions", id = "id")
```

<p style="color:red;">
After displaying of the records, emphasize that for each record, the 
corresponding ID is attached.
</p>

-----

## Special Cases (All for one)
There are a number of nodes that require *special* parsing in the 
*DrugBank* database. in the remainder of the article, we are going to explore 
some of them.


### Reaction (Left, right, enzymes, what??)
The `<reactions>` node is a little complex child that contain many different 
nodes and nodes container as follows:

<p style="color:red;">
Problem: the drug in the `drugbank_record.xml` file does not have reactions in 
it. Either mention that or change the example record in `drugbank_record.xml` 
(which require modifying the XML snippets above to match the content of the 
new drug).
</p>

```{r drug_reactions, echo=FALSE,  fig.width=12}
reactions <- Node$new("<reactions>")
reaction <- reactions$AddChild("<reaction>*")

# reaction <- Node$new("<reaction>")
...
# plotting
SetGraphStyle(reaction, rankdir = "TB")
SetEdgeStyle(reaction, arrowhead = "vee", color = "grey35", penwidth = 2)
SetNodeStyle(reaction, style = "filled,rounded", shape = "box", fillcolor = "GreenYellow",
...
<p style="color:red;">
For some reason, the parent of `<reaction>` (i.e. `<reactions>`) refuses to 
...
that represents the following XML

```{r reaction_xml, eval=FALSE, highlight=TRUE}
<reactions>
    <reaction>
        <sequence>1</sequence>
        <left-element>
...
that parsed in two stages:

* Enzymes are parsed a little as usual:

<p style="color:red;">
Make sure all code is runnable by the reader.
</p>
<p style="color:red;">
Formatting: why are you using a bullet point here?
</p>

```{r reactions_enzymes, eval=FALSE, highlight=TRUE}
reactions_enzymes <- drug_sub_df(.x, "enzymes", id = NULL)
```

* The rest of the nodes which are parsed as follows.

```{r reactions_rest_nodes, eval=FALSE, highlight=TRUE}
...
### ATC (twins, twins, ..)
ATC has repeated node that differe in values.

```{r atc_code, eval=FALSE, highlight=TRUE}
<atc-codes>
...
```{r atrc_parser, eval=FALSE, highlight=TRUE}
...
get_atc_codes_df <- function(rec) {
    return (map_df(xmlChildren(rec[["atc-codes"]]),
                   ~ get_atc_codes_rec(.x, xmlValue(rec["drugbank-id"][[1]]))))
}
```

and this concludes our journey in drug bank database, for more you can visit [dbparser](https://github.com/Dainanahan/dbparser)

I hope that you have enjoyed our journey together.

## About the Authors
We are so and so, co-founded a company Dainanahan whose purpose is to provide
practitioners in the field of drug discovery with useful tools that would make 
their life easier and help increase their productivity. The suite of useful 
tools is called DrugVerse by the way, and dbparser is just the beginning.
