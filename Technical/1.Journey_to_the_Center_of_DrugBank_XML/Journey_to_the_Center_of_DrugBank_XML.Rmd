...
drug_sub_df <- function(rec, main_node, seconadary_node = NULL, id = "drugbank-id") {
  parent_key <- NULL
...
### Optional parameters
We have three optional parameters as follow:
...
articles <- drug_sub_df(.x, "general-references", seconadary_node = "articles")
```

This code parses the following xml snippet
...
#### id
To determine the `parent key` field which is by default `drugbank-id` as follow:
...
There are many special structure that need *special* parsing in **drug bank** database. in the remaining of the article we are going to explore some of them.

### Reaction (Left, right, enzymes, what??)
*Reactions* is a little complex child that contain many different nodes and nodes container as follow:
...
* The rest of nodes which are parsed as follow
...
### ATC (twins, twins, ..)
ATC has repated node that differe in values
...
and that conclude our journey in drug bank database, for more you can visit [dbparser](https://github.com/Dainanahan/dbparser)
