...
<p style="color:red;">
Problem: the code below does not work.
</p>

```{r third_param, eval=FALSE, highlight=TRUE}
# get actions inside carriers nodes, the id here is carrier node
targets <- map_df(all_drugs, ~drug_sub_df(.x, "actions", id = "id"))
```

```{r parse_carriers, eval=FALSE, echo=FALSE}
glimpse(target_actions)
```
```
???
```
...
<p style="color:red;">
After you get the code above to work, fill the ??? above as per the preceding 
examples. Also add below the XML snippet that it is intended to parse (again, 
like the XML snippets added for the previous 'groups' and 'products' examples).
</p>
...
<p style="color:red;">
The `<reactions>` node looks ugly. Fix its appearance if possible.
</p>
